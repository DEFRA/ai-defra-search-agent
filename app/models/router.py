...
async def list_models(
    model_resolution_service: service.AbstractModelResolutionService = fastapi.Depends(dependencies.get_model_resolution_service)
):
    models = model_resolution_service.get_available_models()

    if len(models) == 0:
        raise fastapi.HTTPException(status_code=204, detail="No available models found.")
