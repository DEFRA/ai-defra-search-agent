...
echo "db.knowledgeGroups.drop(); db.knowledgeGroups.insertOne(EJSON.parse(cat('${DATA_DIR}/knowledgeGroups.json')));" | mongosh --host="${MONGO_HOST}" --port="${MONGO_PORT}" "${DB_NAME}"
echo "db.knowledgeSnapshots.drop(); db.knowledgeSnapshots.insertOne(EJSON.parse(cat('${DATA_DIR}/knowledgeSnapshots.json')));" | mongosh --host="${MONGO_HOST}" --port="${MONGO_PORT}" "${DB_NAME}"
