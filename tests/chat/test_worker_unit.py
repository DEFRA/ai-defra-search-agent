...
async def test_process_job_message_client_error_mapping_detailed():
    # More specific mapping assertion for throttling -> 429
    chat_service = AsyncMock()
    response = {
        "Error": {"Code": "ThrottlingException", "Message": "throttle"},
    ...
    # verify that update_message_status was called with a 429 mapping somewhere
    found = False
    for call in conversation_repository.update_message_status.await_args_list:
        # check kwargs first
        if call.kwargs.get("error_code") == 429:
            found = True
        # or positional args (conversation_id, message_id, status, error_message, error_code)
